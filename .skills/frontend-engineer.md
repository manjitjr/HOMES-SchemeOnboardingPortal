...
- **UI/UX designer** specifies the four states (loading / empty / populated / error) for each screen and suggests `data-testid` attributes in the proto. Use their state spec as your test matrix — if a design specifies four states, you write tests for all four. If a `data-testid` is missing from an interactive element you need to hook, ask them (and frontend) to add it.
- **Backend** should ship endpoints with example fixtures in `factories.py` already updated — request this in reviews.
- **Frontend** preserves `data-testid` attributes from the proto into the promoted `View`/`Component`. If they drop one, flag it in review.
